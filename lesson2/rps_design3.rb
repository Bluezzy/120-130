...
class Computer < Player
  ...
  def choose
    case name
    when 'Chameleon' then self.move = Personality::chameleon
    when 'Rock_Solid' then self.move = Personality::rock_solid
    when 'Mister_Random' then self.move = Personality::random
    when 'Unbeatable' then self.move = Personality::unbeatable
    when 'smart' then self.move = Personality::smart
    end      
  end
end
...
class RPSGame
  ...
  def display_winner
    if human.move.greater_than?(computer.move.value)
      @human_score += 1
      puts "#{human.name} won!"
    elsif human.move.weaker_than?(computer.move.value)
      @computer_score += 1
  ...
  def smart_choice
    ...
    if (human_choices.size > 4) && (favorite_choice_percentage >= 66)
      computer.move.value = find_enemy(repetitive_move)
  ...
  def play
    ...
    loop do
      ...
      computer.choose
      @moves_history[computer.name] << computer.move.value
      system 'clear'
      display_moves
      display_winner
      display_score
...
class Personality < RPSGame
  def self.chameleon
    moves_history[-2] || moves_history[-1]
  end
  ...
  def self.unbeatable
    find_enemy(moves_history.last)
  end
  
  def self.smart
    smart_choice || (MOVE::Values.sample)
  end  
end
